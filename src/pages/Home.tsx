...
const HomeContent = ({
  ...
  if (landscape) {
    return (
      <>
        {/* ── Header compacto (sin skyline gigante) ── */}
        <header className="shrink-0 flex items-center justify-between px-4 pt-3 pb-2 gap-3 border-b border-km0-beige-200/60">
          <div className="flex items-center gap-2 min-w-0">
            <img
              src={coatMalgrat}
              alt={`Escudo de ${cityName}`}
              className="w-9 h-9 object-contain shrink-0 drop-shadow-[0_2px_4px_hsl(0_0%_100%/0.5)]"
            />
            <div className="flex flex-col leading-[0.95] min-w-0">
              <h1 className="font-brand font-black text-km0-blue-700 text-base truncate">
                {cityName}
              </h1>
              <Km0Logo className="h-3 w-auto mt-1" />
            </div>
          </div>
          <NotificationBell
            hasAlerts={hasAlerts}
        ...
        {/* ── Cuerpo scrolleable: una sola columna ── */}
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="max-w-[640px] mx-auto px-4 py-3 flex flex-col gap-4">
            {/* 1. Home modules */}
            <HomeModules modules={modules} />

            {/* 2. Iniciar sesión + Registro */}
            ...
            {/* 3. Banner — Promos y eventos destacados */}
            <section>
        ...
        {/* 5. Tab bar inferior */}
        <nav
          className="shrink-0 bg-white border-t border-km0-beige-200 px-2 pt-2 pb-2 grid grid-cols-4"
