...
export interface PointsCardProps {
  ...
  nextLevel: number;
  levelName?: string;
  /** "row" (default): icono+puntos a la izda, barra a la dcha. "compact": apilado vertical. */
  variant?: "row" | "compact";
}
  ...
  if (variant === "compact") {
    return (
      <motion.div
        ...
        className="w-full rounded-2xl bg-white border border-km0-blue-700/10 shadow-[0_8px_24px_-12px_hsl(var(--km0-blue-900)/0.18)] px-3 py-2.5 horizontal-desktop:px-4 horizontal-desktop:py-3 flex flex-col gap-2"
      >
        <div className="flex items-center gap-2">
          <span className="w-8 h-8 horizontal-desktop:w-10 horizontal-desktop:h-10 rounded-full bg-km0-yellow-100 flex items-center justify-center shrink-0">
            <Star
              className="w-4 h-4 horizontal-desktop:w-5 horizontal-desktop:h-5 text-km0-yellow-500 fill-km0-yellow-400"
              strokeWidth={2}
            />
          </span>
          <div className="flex flex-col leading-tight min-w-0">
            <span className="font-brand font-black text-km0-blue-800 text-lg horizontal-desktop:text-2xl">
              {safePoints.toLocaleString("es-ES")}
            </span>
            <span className="font-body text-km0-blue-700/70 text-[10px] horizontal-desktop:text-xs -mt-0.5">
              puntos
            </span>
          </div>
        </div>
        <div className="flex flex-col gap-1">
          <div className="flex items-baseline justify-between gap-2">
            <span className="font-ui font-bold text-km0-blue-800 text-[11px] horizontal-desktop:text-sm">
              {levelName}
            </span>
            <span className="font-body text-km0-blue-700/70 text-[10px] horizontal-desktop:text-xs whitespace-nowrap">
          ...
          <div
            ...
            className="h-1.5 horizontal-desktop:h-2 w-full rounded-full bg-km0-beige-200 overflow-hidden"
          >
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${pct}%` }}
